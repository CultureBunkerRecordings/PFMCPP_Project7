...
void useDefensiveItem(Character* character, Item& item)
{
    ...
    else if( auto* dragslayer = dynamic_cast<DragonSlayer*>(character))
    {
        dragslayer->boostArmor( static_cast<int> (item.getBoost() * 1.5) );
    }
    else if( auto* drag = dynamic_cast<Dragon*>(character) )
    {
...
void useHelpfulItem(Character* character, Item* item)
{
    ...
    {
        dragslayer->boostHitPoints(static_cast<int> (item->getBoost() * 1.25));
    }
    else if( auto* drag = dynamic_cast<Dragon*>(character) )
    {
...
void useAttackItem(Character* character, Item* item)
{
    ...
    else if( auto* dragslayer = dynamic_cast<DragonSlayer*>(character))
    {
        ...
        dragslayer->boostAttackDamage(static_cast<int> (dragslayer->getAttackDamage() * item->getBoost())); 
    } 
    else if( auto* drag = dynamic_cast<Dragon*>(character) )
    {
